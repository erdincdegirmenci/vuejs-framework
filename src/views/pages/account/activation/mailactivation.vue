<template>
  <div>
    <v-container  v-if="activeTemplate === 'redirect'">
      <v-row>
        <v-col
          col="12"
          md="12"
      ...
      <v-container>
        ...
        <v-row>
          <v-col cols="12" md="12">
            ...
            >
              {{ GetLabel(this, "authentication_change_password") }}
            </v-btn>
          </v-col>
        </v-row>
        <hr class="my-4" />
...
export default {
  ...
  data () {
    return {
      ...
      SuccessMesage: this.GetLabel(
        this,
        'authentication_register_complete_message'
      )
    }
  },
  components: {
  ...
  created () {
    if (this.$route.query.Mode === 'register') {
      this.activation()
    } else if (this.$route.query.Mode === 'forgot') {
      this.activeTemplate = 'form'
    } else {
  ...
  methods: {
    changePassword () {
      ...
      this.$store
        .dispatch(CHANGE_PASSWORD_REQUEST, request)
        .then((response) => {
          this.activeTemplate = 'success'
          this.SuccessMesage = this.GetLabel(
            this,
            'authentication_forgot_complete_message'
          )
        })
        .catch((errorCode) => {
          this.activeTemplate = 'success'
    ...
    activation () {
      ...
      this.$store
        .dispatch(ACTIVATION_REQUEST, request)
        .then((response) => {
          this.activeTemplate = 'success'
          this.SuccessMesage = this.GetLabel(
            this,
